...
function saveGarden() {
  ...
  // Magic "SAND"
  header[0] = 83; header[1] = 65; header[2] = 78; header[3] = 68;
  // Version 1
  header[4] = 1;
  // W, H (Uint16) - Little Endian
  ...
  a.href = url;
  a.download = `zen-garden-${new Date().toISOString().slice(0,10)}.sand`;
...
function loadGarden(file) {
  ...
  reader.onload = (e) => {
    try {
      ...
      // Check Magic "SAND"
      if (view.getUint8(0) !== 83 || view.getUint8(1) !== 65 || 
          view.getUint8(2) !== 78 || view.getUint8(3) !== 68) {
      ...
      const newW = view.getUint16(5, true);
      const newH = view.getUint16(7, true);
      
      // Validate dimensions sanity
      if (newW < 100 || newW > 4000 || newH < 100 || newH > 4000) {
        alert("Invalid dimensions in file");
        return;
      }
      
      // Re-init garden with new size
      initGarden(newW, newH);
      
      // Offset 8 bytes for header
      let offset = 8;
      const pixels = newW * newH;
      const byteLen = pixels * 4;
      
      if (buffer.byteLength < 8 + byteLen * 4) {
        alert("File truncated");
        return;
      }
      
      sandHeight.set(new Float32Array(buffer, offset, pixels));
      offset += byteLen;
      sandR.set(new Float32Array(buffer, offset, pixels));
      offset += byteLen;
      sandG.set(new Float32Array(buffer, offset, pixels));
      offset += byteLen;
      sandB.set(new Float32Array(buffer, offset, pixels));
      
      // Regenerate noise for new size
      generateNoiseMap();
      
      // Clear undo stack as state is discontinuous
      undoStack.length = 0;
      redoStack.length = 0;
      updateHistoryBtns();
...
// (No changes to saveGarden/loadGarden, they just handle blobs)
